...
public class PostgreSQLServerHelper extends NullHelper {
	...
	static {
		...
		DEFAULTS.put("'(CONVERT([varchar](5),getdate(),(108)))'",
				"CURRENT_TIME");
	...
	private PostgreSQLServerHelper() {
		System.out
				.println("PostgreSQL helper - setting double slash on UnicodeChecker");
	...
	public String outputType(String type, Integer size) {
		...
		// System.out.println("IPMDBCONV: "+output+" "+type);
		if ("varchar".equalsIgnoreCase(type) && size!=null) {
			if (size > 10485760) {
				output = OUTPUT.get("longvarchar");
				size = null;
			}
		}
	...
	@Override
	public void fixSequences(DBConnection con, String table, String typeName,
	...
	public String normalizeValue(String value) {
		if (value != null) {
			if (value.endsWith("::bpchar")) {
				value = value.substring(1, value.length() - "::bpchar".length()
						- 1);
			} else if (value.endsWith("::character varying")) {
				value = value.substring(1, value.length()
						- "::character varying".length() - 1);
	...
	@Override
	public void setDefaultValue(DBConnection con, String table,
			String typeName, String column, String value) throws Exception {
